...
💡 **高效工作流程**（必须遵守，避免超时）：
1️⃣ 查询天气：
   ✅ 多城市：使用 get_multi_weather（速度快3倍，强烈推荐）
      示例: get_multi_weather({{"cities": ["济南", "青岛", "淄博"]}})
   ⚠️ 单城市：使用 get_weather
      示例: get_weather({{"city": "北京"}})
   
2️⃣ 搜索景点：使用 search_attractions 搜索各城市景点
   - 每个城市1-2次搜索即可
   - 关键词要具体（如"泉水"而非"文化"）
   
3️⃣ 优化顺序：如同城有3+景点，使用 optimize_route（TSP算法）
   - 返回优化后的顺序和直线距离
   - 注意：跨城市景点不要一起优化
   
4️⃣ 规划交通：使用 calculate_route 为关键路段规划交通
   - 跨城市路段：必须规划（使用mode="transit"获取高铁信息）
   - 同城相邻景点：可选规划（距离>2km时建议规划）
   
5️⃣ 搜索配套：使用 search_hotels（每城市1-2次）和 search_food（每城市1次）
...
Thought:{agent_scratchpad}"""
        ...
        agent_executor = AgentExecutor(
            ...
            tools=self.tools,
            verbose=True,
            max_iterations=150,  # 默认150次（会被chat_stream动态调整）
            max_execution_time=180,  # 增加到3分钟（复杂规划需要更多时间）
    ...
    def _estimate_max_iterations(self, user_input: str) -> int:
        ...
        # 计算公式：基础 + 城市数*10 + 天数*4
        # 增加权重是因为每个城市需要多次景点搜索 + optimize_route
        estimated_tools = base_iterations + city_count * 10 + days * 4
        
        # 如果是多城市行程，额外增加路线优化和跨城交通的预留
        if city_count >= 3:
            estimated_tools += 20  # optimize_route(N次POI查询) + 跨城交通规划
        elif city_count >= 2:
            estimated_tools += 10  # 较少的额外工具调用
        
        # 每次工具调用需要2次迭代（Thought + Action），再乘以2.5倍安全边际
        max_iterations = int(estimated_tools * 2 * 2.5)
        
        # 设置上下限（大幅增加上限以支持复杂多城市规划）
        # 实际测试：3城市7天约需21次工具调用 × 2 = 42次迭代
        # 但Agent可能需要多次尝试和调整，所以给3倍空间
        max_iterations = max(40, min(max_iterations, 150))
